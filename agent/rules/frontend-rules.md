...
2. **Client Component** (`_components/*.tsx`):
 ...
 3. **Dosya Yapısı (DOĞRU):**

 ```
 web/app/(main)/module/
 ├── page.tsx                  # Server Component - SADECE data fetching
                               # (API çağrıları, auth kontrolü)
                               # 'use client' KULLANILMAZ!

 web/modules/module-name/
 ├── types.ts                  # TypeScript tipler + Constants
 ├── services.ts               # API servis fonksiyonları
 ├── hooks/                    # Custom hooks (business logic)
 │   └── useModuleHook.ts
 └── components/               # Tüm UI bileşenleri (Client Components)
     ├── ModuleClient.tsx      # Ana client component
     ├── ChildComponent.tsx
     └── index.ts              # Export barrel
 ```

 **ÖNEMLİ: Component Konumu**
 - App klasörü SADECE server-side data fetching yapar
 - Tüm UI bileşenleri `modules/[module]/components/` içinde tutulur
 - `_components` klasörü KULLANILMAZ

 **Örnek - Orders Module:**
 ```
 web/app/(main)/orders/
 └── page.tsx                  # Server: ordersApi.getOrders()...
...
## Date/Time Utility Kuralları

### 1. Relative Time Gösterimi

Sipariş kartları, bildirimler gibi zaman duyarlı UI elementlerinde mutlaka relative time kullanılmalıdır.

```typescript
// web/modules/shared/utils/date.ts
...
**Renk Kodlaması:**
```typescript
export function getRelativeTimeColor(dateString: string): string {
  const diffMinutes = Math.floor((new Date().getTime() - new Date(dateString).getTime()) / 60000)
  
  if (diffMinutes < 15) return 'text-success-main'   // Yeşil - 0-15 dk
  if (diffMinutes < 30) return 'text-warning-main'    // Sarı - 15-30 dk
  if (diffMinutes < 60) return 'text-orange-500'     // Turuncu - 30-60 dk
  return 'text-danger-main'                           // Kırmızı - 60+ dk
}
```

**Kullanım:**
```typescript
import { formatRelativeTime, getRelativeTimeColor } from '@/modules/shared/utils/date'

// Component içinde
<span className={getRelativeTimeColor(order.createdAt)}>
...
export function formatDate(dateString: string, format: 'date' | 'time' | 'datetime' = 'datetime'): string {
  ...
  if (format === 'datetime') {
    return date.toLocaleString('tr-TR', { ...options, hour: '2-digit', minute: '2-digit' })
  }
...
- **Relative time** sadece bugünkü veriler için kullanılır
- **Mutlak tarih** geçmiş tarihler için `formatDate('date')` kullanılır
- **Tüm date utility'leri** `modules/shared/utils/date.ts` içinde tutulur
- **Component içinde** hardcoded date formatı KULLANILMAZ

---

## Kanban Board Kuralları

### 1. Order Grouping (Aynı Masa Çoklu Sipariş)

Kanban board'da aynı masadaki farklı zamanlı siparişler gruplanmalıdır.
...
export interface OrderGroup {
  ...
  totalItems: number
  totalAmount: number
  firstOrderTime: string  // En erken açılış
  lastOrderTime: string   // En son güncelleme
}

// Helper - Siparişleri grupla
export function groupOrdersByTable(orders: Order[]): OrderGroup[] {
...
interface OrderCardProps {
  ...
  onExpand: () => void           // Modal açma
  onStatusChange: (status: OrderStatus) => void
}

// OrderCard render:
// ┌─────────────────────────────────┐
// │ Masa 5                    [↗]  │  ← Expand butonu
// ├─────────────────────────────────┤
// │ • 12:45 - 3 ürün - 245 TL     │  ← Her sipariş ayrı satır
// │ • 12:30 - 2 ürün - 180 TL     │
// │                                 │
// │ TOPLAM: 5 ürün - 425 TL        │
// │ Açılış: 12:25 (25 dk önce)   │  ← Relative time
// └─────────────────────────────────┘
```

### 3. Expand Modal Yapısı

Kart üzerindeki butona tıklayınca açılan modal'da her sipariş ayrı card olarak gösterilir.
...
// Modal içeriği:
// - Her sipariş ayrı kart (ürün listesi detaylı)
// - Toplam hesaplamaları
// - Hızlı aksiyon butonları (Ödeme, Masa Değiştir, İptal)
```

### 4. Kanban Column Yapısı

```typescript
interface KanbanColumnProps {
  status: OrderStatus
  orderGroups: OrderGroup[]   // Order değil OrderGroup!
  onStatusChange: (orderId: string, status: OrderStatus) => void
  onOrderClick: (orderGroup: OrderGroup) => void
}

// Her sütun:
// - Header: Status adı + sipariş sayısı
// - Scrollable order group listesi
// - Drop target (opsiyonel)
```

### 5. Socket Entegrasyonu

Real-time güncellemeler için socket event'leri dinlenmelidir:

```typescript
// useOrdersSocket.ts
export function useOrdersSocket(restaurantId: string) {
  useEffect(() => {
...
| Kural | Açıklama |
|-------|----------|
| Order Group | Aynı masadaki siparişler gruplanır |
| Relative Time | Kart üzerinde mutlaka gösterilir |
| Expand Modal | Her sipariş detaylı görünür |
| Socket | Real-time güncelleme zorunlu |
| Max 200 satır | Her component ayrı dosyada |
