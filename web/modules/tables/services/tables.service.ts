...
export const tablesApi = {
  // Areas
  getAreas: async () => {
    return http.get<Area[]>('/tables/areas')
  },
  ...
  // QR Code Endpoints
  getTableQr: async (tableId: string, restaurantName?: string) => {
  ...
  downloadTableQrPdf: async (tableId: string, restaurantName: string) => {
    const params = new URLSearchParams()
  ...
  getAllTableQrs: async (restaurantName?: string) => {
    const params = new URLSearchParams()
    if (restaurantName) params.append('restaurantName', restaurantName)
    return http.get<TableQrData[]>(`/tables/qr/all?${params.toString()}`)
  },

  downloadAllQrsPdf: async (restaurantName: string) => {
    const params = new URLSearchParams()
    params.append('restaurantName', restaurantName)

    const baseUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8081/api/v1'
    const url = `${baseUrl}/tables/qr/pdf?${params.toString()}`
  ...
  rotateQrCode: async (tableId: string) => {
    return http.post<{ success: boolean; message: string; version: string }>(
