...
export class CreateSchemas1738589400000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`GRANT ALL ON ALL TABLES IN SCHEMA business TO postgres`)
        await queryRunner.query(`GRANT ALL ON ALL TABLES IN SCHEMA operations TO postgres`)
        await queryRunner.query(`GRANT ALL ON ALL TABLES IN SCHEMA public_api TO postgres`)
        await queryRunner.query(`GRANT ALL ON ALL TABLES IN SCHEMA infrastructure TO postgres`)

        // Grant permissions on future tables
        await queryRunner.query(`ALTER DEFAULT PRIVILEGES IN SCHEMA business GRANT ALL ON TABLES TO postgres`)
        await queryRunner.query(`ALTER DEFAULT PRIVILEGES IN SCHEMA operations GRANT ALL ON TABLES TO postgres`)
        await queryRunner.query(`ALTER DEFAULT PRIVILEGES IN SCHEMA public_api GRANT ALL ON TABLES TO postgres`)
        await queryRunner.query(`ALTER DEFAULT PRIVILEGES IN SCHEMA infrastructure GRANT ALL ON TABLES TO postgres`)
    }
