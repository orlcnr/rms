...
export class SuperAdminGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    console.log(
      `[SuperAdminGuard] User: ${user.email}, Role: ${user.role}, RestaurantId: ${user.restaurantId}`,
    );

    // 1. Role kontrolü - SUPER_ADMIN olmalı
    if (user.role !== Role.SUPER_ADMIN) {
      console.log(
        `[SuperAdminGuard] Access Denied: Role ${user.role} is not ${Role.SUPER_ADMIN}`,
      );
    ...
    if (restaurantId !== null && restaurantId !== undefined) {
      console.log(
        `[SuperAdminGuard] Access Denied: Super Admin cannot be tied to a restaurant (${restaurantId})`,
      );
      await this.auditService.emitLog({
