...
export class SuperAdminUsersService {
    ...
    async findAll(queryDto: SearchUserDto) {
        const { page = 1, limit = 20, search, role, restaurant_id, is_active } = queryDto
        ...
        if (restaurant_id) {
            queryBuilder.andWhere('user.restaurant_id = :restaurant_id', { restaurant_id })
        }
    ...
    async create(createUserDto: CreateSuperAdminUserDto) {
        ...
        if (createUserDto.role === Role.SUPER_ADMIN && createUserDto.restaurant_id) {
            throw new ConflictException('SUPER_ADMIN cannot be assigned to a restaurant')
        }

        if (createUserDto.role !== Role.SUPER_ADMIN && !createUserDto.restaurant_id) {
            throw new ConflictException('Non-SUPER_ADMIN users must be assigned to a restaurant')
        }

        // Generate or use provided password
        const password = createUserDto.password || this.passwordService.generateSecurePassword()
    ...
    async update(id: string, updateUserDto: UpdateSuperAdminUserDto) {
        ...
        const newRestaurantId = updateUserDto.restaurant_id !== undefined ? updateUserDto.restaurant_id : user.restaurant_id
        ...
        if (updateUserDto.password) {
            updateUserDto['password_hash'] = await this.passwordService.hashPassword(updateUserDto.password)
    ...
    async updatePassword(id: string, dto: UpdateUserPasswordDto) {
        ...
        const password = dto.password || this.passwordService.generateSecurePassword()
        user.password_hash = await this.passwordService.hashPassword(password)

        await this.userRepository.save(user)

        // Send email
        await this.mailService.sendPasswordUpdateEmail(user.email, user.first_name, password)
