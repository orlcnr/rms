...
export class SuperAdminRestaurantsService {
    ...
    async create(createRestaurantDto: CreateSA_RestaurantDto) {
        ...
        if (existingSlug) {
            throw new ConflictException(`Restaurant with slug '${createRestaurantDto.slug}' already exists`)
        }
        ...
        if (existingUser) {
            throw new ConflictException(`User with email '${createRestaurantDto.owner_email}' already exists`)
        }

        // Generate secure password for owner
        const generatedPassword = this.passwordService.generateSecurePassword()
        const password_hash = await this.passwordService.hashPassword(generatedPassword)

        return await this.restaurantRepository.manager.transaction(async (transactionalEntityManager) => {
            ...
            const savedRestaurant = await transactionalEntityManager.save(restaurant)
    ...
    async update(id: string, updateRestaurantDto: UpdateSA_RestaurantDto) {
        ...
        if (updateRestaurantDto.slug && updateRestaurantDto.slug !== restaurant.slug) {
            const existingSlug = await this.restaurantRepository
                .createQueryBuilder('restaurant')
            ...
            if (existingSlug) {
                throw new ConflictException(`Restaurant with slug '${updateRestaurantDto.slug}' already exists`)
            }
        }
