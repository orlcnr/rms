...
import { PaymentErrorCodes } from '../errors/payment-error-codes';
import { CashErrorCodes } from '../../cash/errors/cash-error-codes';
...
export class PaymentsCommandService {
  ...
  private readonly logger = new Logger(PaymentsCommandService.name);
  ...
  private emitStockDeductionRequested(order: Order): void {
    try {
      this.eventEmitter.emit('payment.stock-deduction.requested', {
        orderId: order.id,
        restaurantId: order.restaurantId,
      });
    } catch (error) {
      this.logger.warn(
        `Failed to emit stock deduction request for order ${order.id}: ${
  ...
  private async updateCustomerVisitStats(
    queryRunner: {
      manager: {
        increment: typeof this.dataSource.manager.increment;
        update: typeof this.dataSource.manager.update;
      };
    },
    restaurantId: string,
    amountByCustomerId: Record<string, number>,
  ): Promise<void> {
    const entries = Object.entries(amountByCustomerId).filter(
      ([customerId, amount]) =>
        Boolean(customerId) && Number.isFinite(amount) && amount > 0,
    );
    ...
    for (const [customerId, amountSpent] of entries) {
  ...
  async create(
    ...
    try {
      ...
      // 3.1 Müşteri istatistiklerini ödeme anında güncelle.
      if (order.customerId) {
        await this.updateCustomerVisitStats(queryRunner, order.restaurantId, {
          [order.customerId]: Number(final_amount),
        });
      }

      // 4. Check if there's an active cash session BEFORE committing
      ...
      this.emitStockDeductionRequested(order);
      ...
      await this.emitDomainAudit({
        ...
        request,
        context: 'PaymentsCommandService.create',
      });
  ...
  async createSplitPayment(
    ...
    try {
      ...
      // 6.1 Müşteri ziyaret istatistiklerini ödeme tamamlandığında güncelle.
      if (order.customerId) {
        await this.updateCustomerVisitStats(queryRunner, order.restaurantId, {
          [order.customerId]: Number(finalOrderTotal),
        });
      } else {
        const amountByCustomerId: Record<string, number> = {};
        for (const payment of payments) {
          if (!payment.customer_id) {
            continue;
          }
          amountByCustomerId[payment.customer_id] =
            (amountByCustomerId[payment.customer_id] || 0) +
            Number(payment.amount || 0);
        }
        await this.updateCustomerVisitStats(
          queryRunner,
          order.restaurantId,
          amountByCustomerId,
        );
      ...
      await this.emitDomainAudit({
        ...
        actor: actor || { id: userId },
        request,
        context: 'PaymentsCommandService.createSplitPayment',
      });
  ...
  async revertPayment(
    ...
    try {
      ...
      if (payment.status === PaymentStatus.REFUNDED) {
        throw new BadRequestException(
          PaymentErrorCodes.PAYMENT_ALREADY_REFUNDED,
        );
      }
      if (payment.payment_method === PaymentMethod.OPEN_ACCOUNT) {
        throw new BadRequestException(
          PaymentErrorCodes.PAYMENT_REFUND_NOT_ALLOWED_FOR_OPEN_ACCOUNT,
        );
      }
      if (payment.status === PaymentStatus.CANCELLED) {
        throw new BadRequestException('Bu ödeme iptal edildiği için iade edilemez');
      }
      const beforeSnapshot = {
      ...
      if (!refundMethod) {
        this.logger.warn(
          JSON.stringify({
            event: 'revert.refund_method.fallback',
            paymentId,
      ...
      if (resolvedRefundMethod === PaymentMethod.OPEN_ACCOUNT) {
        throw new BadRequestException(PaymentErrorCodes.INVALID_REFUND_METHOD);
      }

      const enabledMethods = await this.getEnabledPaymentMethods(
        payment.order?.restaurantId || payment.restaurant_id,
      );
      if (!enabledMethods.has(resolvedRefundMethod)) {
        throw new BadRequestException(PaymentErrorCodes.INVALID_REFUND_METHOD);
      }
      ...
      const activeSession = await queryRunner.manager
        ...
        .innerJoin('session.cashRegister', 'cashRegister')
        .where('session.status = :status', { status: CashSessionStatus.OPEN })
        .andWhere('cashRegister.restaurantId = :restaurantId', {
      ...
      if (!activeSession) {
        throw new BadRequestException(CashErrorCodes.NO_ACTIVE_SESSION);
      }
      ...
      const refundAmount = Number(payment.final_amount ?? payment.amount ?? 0);
      await queryRunner.manager.save(
        CashMovement,
        queryRunner.manager.getRepository(CashMovement).create({
          cashSessionId: activeSession.id,
          type: CashMovementType.OUT,
          subtype: CashMovementSubtype.REFUND,
          paymentMethod: resolvedRefundMethod,
          amount: refundAmount,
          description: `İade - Sipariş #${payment.order_id}${reason ? ` (${reason})` : ''}`,
          userId: actor?.id || payment.order?.userId || undefined,
          orderId: payment.order_id,
          referencePaymentId: payment.id,
        }),
      );

      // Status update sonrası aggregate çekilip sipariş durumu çözülür.
      const aggregate = await queryRunner.manager
        ...
        .where('payment.order_id = :orderId', { orderId: payment.order_id })
        .setParameters({
      ...
      if (payment.order) {
        const totalCompleted = Number(aggregate?.totalCompleted ?? 0);
        const totalRefunded = Number(aggregate?.totalRefunded ?? 0);
        const netPaid = totalCompleted - totalRefunded;
        const orderFinalAmount = Number(payment.order.totalAmount ?? 0);
        payment.order.status =
          netPaid >= orderFinalAmount
            ? OrderStatus.PAID
            : OrderStatus.PENDING;
      ...
      await this.emitDomainAudit({
        ...
        actor,
        request,
        context: 'PaymentsCommandService.revertPayment',
      });
