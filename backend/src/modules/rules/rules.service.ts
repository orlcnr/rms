...
export class RulesService implements OnModuleInit {
  ...
  async checkRule(
    ...
    if (!evaluator) {
      this.logger.warn(
        `No evaluator found for category: ${rule.category} (Rule: ${key})`,
      );
      return;
    }
    ...
    if (!isValid) {
      this.logger.warn(
        `Business Rule Violated: [${key}] for Restaurant [${restaurantId}]. Context: ${JSON.stringify(context)}`,
      );
      throw new BadRequestException(
        errorMessage ||
          rule.name ||
          'Bu işlem kural kısıtlaması nedeniyle gerçekleştirilemez.',
      );
    }

    this.logger.log(
      `Business Rule Passed: [${key}] for Restaurant [${restaurantId}]`,
    );
  }
  ...
  async toggleRule(
    ...
    if (!rule) {
      this.logger.error(
        `Attempted to toggle non-existent rule: [${key}] for Restaurant [${restaurantId}]`,
      );
    ...
    this.logger.log(
      `Business Rule [${key}] toggled to [${isEnabled}] for Restaurant [${restaurantId}]`,
    );
    return updatedRule;
  }
  ...
  async initializeDefaultRules(restaurantId: string): Promise<void> {
    const defaultRules = [
      ...
      },
      {
        ...
        description:
          'Hazırlık aşamasına geçmiş siparişlerin iptal edilmesini kısıtlar.',
        is_enabled: true,
      },
      {
        ...
        description:
          'İşlem görmüş (stok hareketi olan) malzemelerin silinmesini engeller.',
        is_enabled: true,
      },
      {
    ...
    this.logger.log(
      `Default Business Rules initialized for Restaurant [${restaurantId}]`,
    );
  }
}
